...
# Настройки
TARGET_URL = sys.argv[1]   # Цель атаки
THREADS = int(sys.argv[2])  # Количество потоков
DURATION = int(sys.argv[3])  # Время тестирования в секундах
UAs = [
...
# Функция, отправляющая запросы с прокси
def send_requests(stop_event):
    ...
    while not stop_event.is_set():
        try:
            ...
            print(f"[{threading.current_thread().name}] {response.status_code} - {response.reason}")
...
def start_stress_test():
    ...
    def thread_wrapper(thread_id):
        while not stop_event.is_set():
            thread = threading.Thread(target=send_requests, args=(stop_event,), name=f"Thread-{thread_id}")
            threads[thread_id] = thread
            thread.start()
            thread.join()  # Ожидаем завершения, если поток завершился с ошибкой, он перезапустится

    # Создаем потоки и запускаем их в обертке
    for i in range(THREADS):
        thread_id = i + 1
        monitoring_thread = threading.Thread(target=thread_wrapper, args=(thread_id,))
        monitoring_thread.daemon = True  # Для завершения всех потоков при остановке основного
...
if __name__ == "__main__":
    print("Запуск HTTP стресс-тестера с перезапуском потоков...")
    start_time = time.time()
    start_stress_test()
    print(f"Тест завершён за {time.time() - start_time:.2f} секунд.")
